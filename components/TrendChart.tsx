...
export default function TrendChart({ tickets }: { tickets: Ticket[] }) {
  ...
  const data = useMemo(() => {
    ...
    keys.forEach((k) => { map[k] = { date: k, tickets: 0, resolved: 0, total: 0, avgRes: 0 }; });
    filtered.forEach((t) => {
      if (!t.createdOn) return;
      const k = getTicketKey(t.createdOn, period);
      if (!map[k]) return;
      map[k].tickets++;
      if (isResolved(t)) { map[k].resolved++; map[k].total += t.workingResolutionMin; }
    });
    return Object.values(map).map((d) => ({ ...d, avgRes: d.resolved > 0 ? Math.round(d.total / d.resolved) : 0 }));
  }, [tickets, period]);
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={270}>
        <LineChart data={data} margin={{ top: 4, right: 24, left: 0, bottom: period !== "week" ? 30 : 0 }}>
          ...
          <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 11, fill: "var(--text-muted)" }} tickFormatter={(v) => formatMinutes(v as number)} axisLine={false} tickLine={false} />
          ...
          <Line yAxisId="right" type="monotone" dataKey="avgRes"   name="Avg Resolution" stroke="#e07b39" strokeWidth={2} strokeDasharray="5 3" dot={false} activeDot={{ r: 4 }} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
