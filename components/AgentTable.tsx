...
export default function AgentTable({ tickets }: Props) {
  ...
  const agents: AgentStats[] = useMemo(() => {
    ...
    return Object.values(map).map((a) => ({
      ...a,
      avgRes: a.resolved > 0 ? Math.round(a.sumRes / a.resolved) : null,
    }));
  }, [tickets]);
  ...
  const resRateBadge = (rate: number) => {
  ...
  return (
    <div style={{ overflowX: "auto" }}>
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
        <thead>
          <tr>
            <th style={{ ...thStyle("total"), cursor: "default" }}>Agent</th>
            <th style={thStyle("total")}    onClick={() => setSortBy("total")}>
              Tickets {sortBy === "total" ? "↓" : ""}
            </th>
            <th style={thStyle("resolved")} onClick={() => setSortBy("resolved")}>
              Resolved {sortBy === "resolved" ? "↓" : ""}
            </th>
            <th style={thStyle("resRate")}  onClick={() => setSortBy("resRate")}>
              Res. Rate {sortBy === "resRate" ? "↓" : ""}
            </th>
            <th style={thStyle("avgRes")}   onClick={() => setSortBy("avgRes")}>
              Avg Resolution {sortBy === "avgRes" ? "↑" : ""}
            </th>
          </tr>
        </thead>
        <tbody>
            ...
            >
              ...
              <td style={{ padding: "11px 14px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <span style={{ minWidth: 28, textAlign: "right", fontWeight: 700 }}>
                    {a.total}
                  </span>
                </div>
              </td>
              ...
              {/* Resolved */}
              <td style={{ padding: "11px 14px", color: "#2d9c6b", fontWeight: 700 }}>
                {a.resolved}
              </td>

              {/* Resolution rate */}
              <td style={{ padding: "11px 14px" }}>
                {resRateBadge(a.resolved / a.total)}
              </td>

              {/* Avg working-hours resolution */}
              <td
                style={{
                  padding: "11px 14px",
