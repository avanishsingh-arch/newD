...
export default function DashboardPage() {
  ...
  const filtered = useMemo(() => filterByDays(allTickets, trendDays), [allTickets, trendDays]);

  const stats = useMemo(() => {
    const resolved = filtered.filter(isResolved);
    const avgRes   = resolved.length > 0
      ? Math.round(resolved.reduce((s, t) => s + t.workingResolutionMin, 0) / resolved.length)
      : null;
    ...
    return {
      ...
      open,
      avgRes,
      resRate: filtered.length > 0 ? Math.round((resolved.length / filtered.length) * 100) : 0,
    };
  }, [filtered]);
        ...
        {!loading && !error && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 12, marginBottom: 28 }}>
              <StatCard label={`Total Tickets (L${trendDays})`} value={stats.total}    accent="#3266ad" />
              <StatCard label="Resolved"    value={stats.resolved} sub={`${stats.resRate}% resolution rate`} accent="#2d9c6b" />
              <StatCard label="Open / Pending" value={stats.open} accent="#e07b39" />
              <StatCard label="Avg Resolution Time" value={stats.avgRes != null ? formatMinutes(stats.avgRes) : "—"} sub="working hours only" accent="#9b4dca" />
            </div>
